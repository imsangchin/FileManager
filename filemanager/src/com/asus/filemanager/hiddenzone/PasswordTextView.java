...
public class PasswordTextView extends View {
    public static final int INDEX_PREVIOUS = -1;
     ...
     */
    private final float mTextHeightRaw;
    private float mMaxTextHeight;
    private float mApplyTextHeight;
    ...
    public PasswordTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mTextHeightRaw = context.getResources().getDimension(R.dimen.lockscreen_password_font_size);
            // a.getInt(R.styleable.PasswordTextView_scaledTextSize, 0);
        } finally {
            a.recycle();
        }
        ...
        if(ThemeUtility.getThemeType() == ThemeUtility.THEME.DARK) {
            mDrawPaint.setColor(getResources().getColor(R.color.dark_theme_hiddenzone_password_color));
        } else {
           mDrawPaint.setColor(getResources().getColor(R.color.hiddenzone_password_color));
        }
        mDrawPaint.setTypeface(Typeface.create("sans-serif", 0));
        mDotSize = getContext().getResources().getDimensionPixelSize(R.dimen.password_dot_size);
        mCharPadding = getContext().getResources().getDimensionPixelSize(R.dimen.password_char_padding);
        mShowPassword = Settings.System.getInt(getContext().getContentResolver(),
                Settings.System.TEXT_SHOW_PASSWORD, 1) == 1;
        mAppearInterpolator = AnimationUtils.loadInterpolator(getContext(),
                android.R.interpolator.linear);
        mDisappearInterpolator = AnimationUtils.loadInterpolator(getContext(),
                android.R.interpolator.linear);
    ...
    protected void onDraw(Canvas canvas) {
        if (mMaxTextHeight == 0) {
            mMaxTextHeight = getHeight();
            mApplyTextHeight = Math.min(mTextHeightRaw, mMaxTextHeight);
        }
    ...
    // ++ Wesley
    public void removeLastCharAnimation() {
        int size = mTextChars.size();
        if (size != 0) {
            CharState charState = mTextChars.get(size - 1);
    ...
    private Rect getCharBounds() {
        mDrawPaint.setTextSize(mApplyTextHeight);
